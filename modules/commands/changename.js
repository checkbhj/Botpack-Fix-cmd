...
version: "3.0.0",
hasPermssion: 1,
credits: global.config.Codemaker, //english by 𝚉𝚒𝚊 𝚁𝚎𝚒𝚗
...
const permission = [`100071575183469`,``];
if (!permission.includes(event.senderID)) return api.sendMessage("You don't have permission to use this command.", event.threadID, event.messageID);
    ...
    if (custom.length != 0) {
        ...
        for (const idThread of allThread) {
            ...
            api.changeNickname(`「 ${(threadSetting.hasOwnProperty("PREFIX")) ? threadSetting.PREFIX : global.config.PREFIX} 」➭ ${(!global.config.BOTNAME) ? "" : global.config.BOTNAME}`, idThread.threadID, idBot, (err) => (err) ? threadError.push(idThread.threadID) : '');
            count+=1;
            await new Promise(resolve => setTimeout(resolve, 500));
        }
        return api.sendMessage(`Successfully renamed for ${count} group`, event.threadID, () => {
