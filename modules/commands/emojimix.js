...
if (!args[0])
    return api.sendMessage("Wrong format!\nUse "+global.config.PREFIX+this.config.name+" "+this.config.usages, event.threadID, event.messageID);
	 ...
	 return request(encodeURI(`${APIURL}/api/maker/emojimix?emoji1=${emoji1}&emoji2=${emoji2}&apikey=${APIKEY}`)).pipe(fs.createWriteStream(__dirname+'/cache/biden.png')).on('close',() => callback()); 
} catch (err){
