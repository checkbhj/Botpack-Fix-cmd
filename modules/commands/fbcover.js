module.exports.config = {
  ...
  cooldowns: 0,
  usages: "facebook cover photo make",
  dependencies: {
    "fs-extra": "",
    "request": "",
    "axios": ""
...
if ((this.config.credits) != "SABIK") { return api.sendMessage(`[ WARNING ] - Don't credits modules ${this.config.name} was changed to ${this.config.credits} by ADMINBOT ${global.config.BOTNAME} 😐 Stop now!!!`, event.threadID, event.messageID)}
    const request = require('request');
  const fs = require("fs-extra")
  const axios = require("axios")
  const { threadID, messageID, senderID, body } = event;
  if(!args[0]) return api.sendMessage('➡️Please enter primary name!!!', threadID, messageID)
...
module.exports.handleReply = async function({ api, event, args, handleReply, client, __GLOBAL, Threads, Users, Currencies }) {
  ...
  switch (handleReply.type) {
    ...
    case "sdt": {
      api.unsendMessage(handleReply.messageID);
      return api.sendMessage(`➡️You have selected SDT as: ${event.body.toUpperCase()}\n\n(Reply this message to enter your email)`,threadID, function (err, info) {
        return global.client.handleReply.push({
          type: "email",
          name: `fbcover`,
          author: senderID,
    ...
    case "create": {
      ...
      api.unsendMessage(handleReply.messageID);
      api.sendMessage(`Your Cover Almost Done Please Wait⏳ Initializing the imager!`,threadID, (err, info) => {
      setTimeout(() => {
